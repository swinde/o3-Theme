[{block name="widget_header_search_form"}]
    <form class="form search" id="searchForm" role="search" action="[{$oViewConf->getSelfActionLink()}]" method="get" name="search">
        ...
        [{block name="dd_widget_header_search_form_inner"}]
            <div class="input-group search-suggest-wrapper">
                ...
                [{block name="header_search_field"}]
                    <input class="form-control" type="search" id="searchParam" name="searchparam" value="[{$oView->getSearchParamForHtml()}]" placeholder="[{oxmultilang ident="SEARCH"}]" autocomplete="off" data-suggest-url="[{$oViewConf->getSelfLink()}]">
                [{/block}]

                [{block name="dd_header_search_button"}]
                    <button type="submit" class="btn header__search-button" id="searchSubmit" title="[{oxmultilang ident="SEARCH_SUBMIT"}]">
    ...
    [{if $oxcmp_shop->oxshops__oxproductive->value}]
        [{oxscript include="js/widget/oxsearchsuggest.min.js" priority=10}]
    [{else}]
        [{oxscript include="js/widget/oxsearchsuggest.js" priority=10}]
    [{/if}]
[{/block}]
